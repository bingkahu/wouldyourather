// _worker.js — Cloudflare Pages + D1 compatible

// Tell Cloudflare Pages to serve static files itself.
// Worker will ONLY run for /api/* routes.
export const config = {
  runtime: "edge",
  exclude: [
    "/",
...
async function getUserFromSession(request, env) {
  ...
  return await env.DB.prepare(
    "SELECT id, username, dark_mode FROM users WHERE id = ?"
  ).bind(userId).first();
}
...
async function apiSubmit(request, event) {
  ...
  if (!option_a || !option_b)
    return jsonResponse({ error: "Missing options" }, 400);
